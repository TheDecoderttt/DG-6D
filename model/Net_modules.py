...
class AttnBlock(nn.Module):
    def __init__(self, d_model=256, num_heads=4, dim_ffn=256, dropout=0.0, dropout_attn=None, use_pos_enc=False):
    ...
    def forward(self, kpt_query, input_feature, pos_enc=None):
        # Add positional encoding if needed
        if self.use_pos_enc:
            kpt_query = self.with_pos_embed(kpt_query, self.pos_enc)
            input_feature = self.with_pos_embed(input_feature, self.pos_enc)
        
        # pos = self.pos_encoder(input_feature)
        # input_feature = self.with_pos_embed(input_feature, pos)
...
class InstanceAdaptiveKeypointDetector(nn.Module):
    def __init__(self, cfg):
        ...
        self.glo_enhance_query = global_enhancev2()
    ...
    def forward(self, rgb_local, pts_local):
        ...
        #############enhance 2 [36,96,256]
        batch_kpt_query = self.glo_enhance_query(batch_kpt_query)
...
class PoseSizeEstimator(nn.Module):
    ...
    def forward(self, pts1, pts2, pts1_local):
        ...
        #     x_i = pts_so[i].unsqueeze(1)  # [b, 1, N]
        #     x_i = x_i.transpose(0, 1)  #[96,1,3]->[1,96,3]
        #     x_i = self.vnn(x_i).unsqueeze(0)  #[1,192,96]
        #     x_list = torch.cat((x_list, x_i), dim=0)
        
        # pts_so = x_list  # [12, 192, 96])
        # pts_so = self.pts_so_mlp1(pts_so)

        pts1 = self.pts_mlp1(pts1.transpose(1,2))
        pts2 = self.pts_mlp2(pts2.transpose(1,2))
        # pose_feat = torch.cat([pts1, pts1_local.transpose(1,2), pts2, pts_so], dim=1) # b, c, n
