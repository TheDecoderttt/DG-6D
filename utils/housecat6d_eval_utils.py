...
def compute_independent_mAP(final_results, synset_names, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=True, logger=None):
    ...
    # plot_mAP(iou_3d_aps,pose_aps, out_dir='./log/housecat6d/mAP',iou_thres_list=[i/100 for i in range(101)],
    #                         degree_thres_list=list(range(0,61,1)),
    #                         shift_thres_list=[i/2 for i in range(21)],)
    plot_mAP(iou_3d_aps,pose_aps, out_dir='./log/housecat6d/mAP',iou_thres_list=  iou_3d_thresholds,
...
def plot_mAP(
    ...
    labels = ['box','bottle', 'can', 'cup', 'remote', 'teapot', 'cutlery', 'glass', 'shoe','tube','mean', 'nocs']
    colors = ['tab:blue', 'tab:orange', 'tab:green', 'tab:pink', 'tab:olive', 'tab:purple', 'tab:cyan','tab:red', 'tab:gray','#FF6347','#87CEEB','#32CD32']
    styles = ['-', '-', '-', '-', '-', '-','-','-','-','-','-',':' ]
...
def evaluate_housecat(path, logger=None):
    ...
    compute_independent_mAP(final_results, synset_names,
                            ...
                            shift_thresholds=[i / 2 for i in range(21)],
                            iou_3d_thresholds=[i / 100 for i in range(101)], logger=logger)
